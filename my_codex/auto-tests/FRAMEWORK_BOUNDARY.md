# FRAMEWORK_BOUNDARY.md - Контракт для AI

Данный репозиторий представляет собой проект для автоматизации тестирования системы.

## Поведение при нехватке функциональности
Если для реализации тест-кейса тебе не хватает метода в `APIHandler` (например, нет метода `contract`), ты **НЕ ПИШЕШЬ ЕГО САМ**. 
Ты делаешь следующее:
1. В папке задачи создаешь файл `FRAMEWORK_GAPS.md`.
2. Записываешь туда: `Для теста X требуется метод CONTRACT /users/{id}. В текущем API-клиенте он отсутствует. Предлагаю реализовать его вручную разработчикам фреймворка.`
3. Ты пропускаешь этот конкретный тест (ставишь `@Disabled("Нет метода в API")`), но остальные тесты по задаче автоматизируешь тем, что есть.
